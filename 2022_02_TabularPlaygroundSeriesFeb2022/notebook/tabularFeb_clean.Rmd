---
title: "cleaning for Kaggle challenge: Tabular Playground Series - Feb 2022"
...
nearZero_info <- caret::nearZeroVar(df_raw, freqCut = 4, uniqueCut = 1.6, saveMetrics = TRUE) %>%
...
```{r}
df <- df_raw %>% select(-which(nearZero_info$nzv))
```
...
```{r}
df %>% write_csv(file = '../data/train_clean.csv')
```
